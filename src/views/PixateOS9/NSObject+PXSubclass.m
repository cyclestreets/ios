...
static BOOL respondsToSelectorIMP(id self, SEL _cmd, SEL selector)
{
    ...
    static BOOL applyHack=NO;
    static BOOL hasBeenSet=NO;
    
    if(hasBeenSet==NO){
        if([DeviceUtilities isSimulatorDevice]){
            //  BetterLog(@"");
            if(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"9.0")){
                //   BetterLog(@"");
                if ([DeviceUtilities is64BitDevice]) {
                    applyHack=YES;
                }
            }
        }
    ...
    if(applyHack==YES){
       ...
       return ((BOOL)callSuper1v(self, [self pxClass], _cmd, selector)) || (class_getInstanceMethod(object_getClass(self), selector) != NULL);
    }
