...
+ (void)setPathToRoundedRect:(CGRect)rect forInset:(NSUInteger)inset inContext:(CGContextRef)context forRadius:(int)radius
{
	...
	CGFloat d = (inset<radius)?(radius-inset):0;
	d = (d>0.5*m)?(0.5*m):d;
...
- (void)drawRect:(CGRect)rect
{
	...
	if(cornerRadius>0){
		[GlossyGradientView setPathToRoundedRect:CGRectMake(0, 0, size.width, size.height) forInset:0 inContext:context forRadius:cornerRadius];
