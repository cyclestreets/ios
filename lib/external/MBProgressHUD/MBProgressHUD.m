...
- (void)setTouchToContinueLabelText:(NSString *)newText {
	if ([NSThread isMainThread]) {
		[self updateDetailsLabelText:newText];
		[self setNeedsLayout];
		[self setNeedsDisplay];
...
- (NSString *)touchToContinueLabelText {
	return touchToContinueLabelText;
}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
		...
		self.touchToContinue = NO;
		self.touchToContinueLabelText = nil;
		...
		touchToContinueLabel = [[UILabel alloc] initWithFrame:self.bounds];
		taskInProgress = NO;
...
- (void)layoutSubviews {
	...
	CGRect lFrame	= CGRectZero;  //title label frame
	CGRect lFrameD	= CGRectZero; // description label frame
	CGRect lFrameT	= CGRectZero; // touch to continue label frame
	CGSize dims ;
	float lHeight;
	float lWidth;
	...
	if (nil != self.detailsLabelText) {
		...
		lFrameD = CGRectMake(floor((frame.size.width - lWidth) / 2) + xOffset,
	...
	if (self.touchToContinue==YES) {
		// Get size of label text
		dims = [self.touchToContinueLabelText sizeWithFont:self.detailsLabelFont];
		
		// Compute label dimensions based on font metrics if size is larger than max then clip the label width
		lHeight = dims.height + (4 * PADDING);
		if (dims.width + (4 * PADDING) <= (frame.size.width - (2 * MARGIN))) {
			lWidth = dims.width + (4 * PADDING);
		}
		else {
			lWidth = frame.size.width - (4 * MARGIN);
		...
		// Set label properties
		touchToContinueLabel.font = self.detailsLabelFont;
		touchToContinueLabel.numberOfLines=0;
		touchToContinueLabel.adjustsFontSizeToFitWidth = NO;
		touchToContinueLabel.textAlignment = UITextAlignmentCenter;
		touchToContinueLabel.opaque = NO;
		touchToContinueLabel.backgroundColor = [UIColor clearColor];
		touchToContinueLabel.textColor = [UIColor whiteColor];
		touchToContinueLabel.text = self.touchToContinueLabelText;
		touchToContinueLabel.layer.cornerRadius = 8.0;
		touchToContinueLabel.layer.borderColor = [UIColor whiteColor].CGColor;
		touchToContinueLabel.layer.borderWidth = 1.0;
		...
		// Move first label to make room for the new label
		lFrameD.origin.y -= (floor(lHeight / 2 + PADDING / 2));
		detailsLabel.frame = lFrameD;
		
		// Set label position and dimensions
		lFrameT = CGRectMake(floor( (frame.size.width - lWidth) / 2) + xOffset,
							 lFrame.origin.y + lFrame.size.height + lFrameD.size.height + (lFrameD.size.height>0?PADDING:0) + (2*PADDING), lWidth, lHeight);
